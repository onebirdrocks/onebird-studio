...
const MainLayout: FC = () => {
  ...
  return (
    <div className="flex h-screen bg-white dark:bg-slate-900">
      {/* 左侧功能导航 */}
      <nav className="w-16 bg-gray-100 dark:bg-slate-800 flex flex-col items-center py-4">
        <div className="flex-1 flex flex-col items-center space-y-4">
          ...
          {/* 顶部导航项 */}
          {navItems.filter(item => item.position === 'top').map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={cn(
        ...
        {/* 底部导航项 */}
        {navItems.filter(item => item.position === 'bottom').map((item) => (
          <Link
            key={item.path}
            to={item.path}
            className={cn(
